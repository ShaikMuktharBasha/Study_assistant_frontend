...
import { Mail, Lock, ArrowRight, Zap, BookOpen, MessageSquare, Loader2 } from 'lucide-react';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL || 'https://study-assistant-backend-seven.vercel.app'}/api/auth/login`, { email, password });
  ...
  return (
    <div className="flex min-h-[calc(100vh-70px)] bg-white dark:bg-[#0A0A0A] transition-colors duration-500 font-sans">
      ...
      <div className="hidden lg:flex w-1/2 relative bg-[#050511] overflow-hidden flex-col justify-between p-16">
        ...
        <div className="absolute top-0 right-0 w-[40rem] h-[40rem] bg-indigo-600/30 rounded-full mix-blend-screen filter blur-[150px] transform translate-x-1/3 -translate-y-1/4 animate-pulse duration-10000"></div>
        <div className="absolute bottom-0 left-0 w-[35rem] h-[35rem] bg-purple-600/20 rounded-full mix-blend-screen filter blur-[150px] transform -translate-x-1/3 translate-y-1/4"></div>
        ...
        <div className={`relative z-10 space-y-4 max-w-xl transition-all duration-1000 transform ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-indigo-300 text-sm font-medium backdrop-blur-md mb-4 shadow-xl">
            <Zap size={14} className="text-yellow-400 fill-current" /> AI-Powered Platform
          </div>
          <h1 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 tracking-tight leading-tight">
            Elevate Your <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400">Study Experience.</span>
          </h1>
          <p className="text-gray-400 text-lg leading-relaxed pt-2 font-medium">
            Transform static documents into dynamic, interactive learning sessions. Let AI summarize, quiz, and chat with you about your materials.
          </p>
        </div>
      ...
      {/* Right side - Login Form (Ultra sleek white/dark mode) */}
        ...
        <div className={`w-full max-w-md transition-all duration-1000 delay-100 transform ${mounted ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
          <div className="text-center mb-10">
            ...
            <p className="mt-3 text-gray-500 dark:text-gray-400 font-medium">
              ...
              <Link to="/register" className="font-bold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 hover:underline transition-all">
                Sign up for free
              </Link>
            </p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={loading}
                className="group relative w-full flex items-center justify-center gap-2 py-4 px-4 border border-transparent text-sm font-bold rounded-2xl text-white bg-gray-900 dark:bg-white dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-100 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 dark:focus:ring-offset-[#0A0A0A] transition-all duration-300 hover:-translate-y-0.5"
              >
                {loading ? (
                  <>
